...
require 'octopodes/models'

include Octopodes::Models
  ...
  module Records
    class Datastore
      ...
      def self.trans_form_data(decoded_www_form)
        decoded_www_form.each_with_object({}) do |value, hash|
    ...
    class Users < Datastore
      ...
      def self.update(_token, username, data)
        first_user_doc = find(username).first

        unless first_user_doc.empty?
      ...
      def self.create_from_collection(token, collection)
        data = collection.template.data.each_with_object({}) do |cj_data, hash|
          nv = cj_data.to_hash
      ...
      def self.check_auth(username, password)
        ...
        if !username.nil? && !password.nil?
          ...
          unless first_user_doc.empty?
            ...
            user = { username: identity.username } if is_authorized
          end
        end
    ...
    class RecordSet
      ...
      def last_key
        @documents['rows'].last['key'] if count >= 1
      end
    ...
    class CreativeWorks < Datastore
      def self.new
        RecordSet.new.tap { |r| r.add(model.new) }
      end
      ...
      def self.update(id, _edit, data = {}, username)
        save_and_make_recordset(id, data, username)
      end
      ...
      def self.all(options = {})
        ...
        options[:startkey] && params << ['startkey', options[:startkey]]

        options[:limit] && params << ['limit', options[:limit] + 1]

        uri = design_uri({ design: 'all', view: design_doc(:all) }, params)
        fetch_recordset(uri)
      end

      def self.find(id)
        key = model.id_prefix + id
        params  = [[:reduce, 'false'], [:include_docs, 'true'],
                   ['startkey', "[\"#{key}\"]"], ['endkey', "[\"#{key}\", {}]"]]
        uri = design_uri({ design: 'all', view: design_doc(:find) }, params)
        fetch_recordset(uri)
      end
      ...
      def self.history(id)
        ...
        params  = [[:reduce, 'false'], [:include_docs, 'true'],
                   ...
                   ['descending', 'true']]
        uri = design_uri({ design: 'all', view: design_doc(:history) }, params)
        fetch_recordset(uri)
      end
      ...
      def self.make_work(id, data = {}, username)
        safe_data = model.whitelist(data)
        existing_work = find(id).items.first

        if existing_work.nil?
          work = model.new(safe_data)
          work.slug = id
        else
      ...
      def self.try_to_version(creative_work)
        ...
        version_and_save(existing_work) unless existing_work.nil?
      end
    ...
    class Documents
      ...
      def cj_item_link(hash, key, rel, prompt)
        { href: hash[key], rel: rel, prompt: prompt } if hash.key?(key)
      end
    end
    ...
    class SignupDocuments < Documents
      ...
      def items
        @items ||= (@documents['rows'] || []).map do |row|
          ...
          data = [{ name: 'message', prompt: 'Message',
                    value: 'Your registration was successful. You may now login.' }]
          links = []
    ...
    class WebPageDocuments < Documents
      ...
      def items
        @items ||= (@documents['rows'] || []).map do |row|
          ...
          reviewed_by = doc.key?('reviewedBy') ? doc['reviewedBy'] : {}
          ...
          data = [cj_item_datum(part, 'name', 'name', 'Title'),
                  ...
                  cj_item_datum(reviewed_by, '@id', 'reviewedBy', 'Reviewed By'),
